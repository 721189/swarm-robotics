...
Equally instructive is what does \emph{not} degrade: per-link PDR follows the analytic channel model independently of swarm logic (Table~\ref{tab:correlation}), and cohesion remains stable across radio conditions once assignments exist.
...
D-TDMA connects to demand-assigned MAC protocols in tactical networks, but to our knowledge its coupling to auction convergence---and the doubled-horizon condition $2N\tau \leq T_{\mathrm{val}}$---is new.
...
All agents share parameters, battery attrition is disabled in the benchmark corpus, so the fault-tolerance path of ghost pruning is exercised only through belief expiry rather than true agent loss.
