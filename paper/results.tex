...
We report results from the released corpus of $4{,}860$ TDMA-CBAA trials ($324$ configurations, $15$ independent seeds each; Sec.~\ref{sec:test-matrix}) together with the four-baseline ablation sweep ($900$ runs) and the communication--motion isolation ablation of Sec.~\ref{sec:setup-ablation}.
...
\begin{table}[htbp]
\centering
\caption{Headline metrics over 4{,}860 TDMA-CBAA trials (mean $\pm$ SEM, bootstrap 95\% CI). MTC is right-censored at 50\,s.}
\label{tab:headline}
\begin{tabular}{@{}lccc@{}}
\toprule
...
The aggregate PDR of $0.827$ is consistent with the two-stage loss model of Sec.~\ref{sec:method-loss}: averaging the per-loss predictions $1.0,\ 0.926,\ 0.786,\ 0.595$ over the corpus's loss mixture reproduces $0.827$, and per-condition errors are below $10^{-3}$ (Sec.~\ref{sec:results-pdr}), providing an end-to-end sanity check that the radio simulator behaves as designed.
...
Because the ablation holds the topology fixed ($M{=}5$, $k{=}2$, $\tau{=}50$\,ms) and varies only $N \times p_{\mathrm{tx}}$ with 30 seeds per condition, whereas Table~\ref{tab:headline} aggregates the full 324-configuration factorial at 15 seeds, the two tables' absolute values are not directly comparable; the differing sampling explains the modest shift in TDMA-with-loss MTC ($22.4$\,s here vs.\ $21.6$\,s corpus mean) while the qualitative ordering across baselines is preserved.

\begin{table}[htbp]
\centering
\caption{Four-baseline ablation ($N \in \{10,30,50\}$; $M{=}5$, $k{=}2$, $\tau{=}50$\,ms fixed; 30 independent seeds per condition). MTC is right-censored at 50\,s.}
\label{tab:ablation}
\begin{tabular}{@{}lccc@{}}
\toprule
Baseline & MTC (s) & Convergence & PDR \\
\midrule
Ideal CBAA            & $0.34$ & $100\%$   & $1.000$ \\
Loss-only (open ch.)  & $0.47$ & $100\%$   & $0.829$ \\
TDMA-only             & $3.46$ & $100\%$   & $1.000$ \\
TDMA-CBAA (proposed)  & $22.4$ & $61.4\%$  & $0.828$ \\
\bottomrule
\end{tabular}
\end{table}
