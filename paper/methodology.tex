\section{Methodology: TDMA-CBAA}\label{sec:methodology}

TDMA-CBAA interleaves the CBAA single-task consensus auction with a slotted radio discipline and belief management.
Algorithm~\ref{alg:tdmacbaa} summarizes one simulation frame; the components relative to canonical CBBA---slotted medium access, two-stage packet loss, and belief expiry---are detailed in Sections~\ref{sec:method-slotted}--\ref{sec:method-belief}, followed by the D-TDMA extension in Section~\ref{sec:dtdma}.

\begin{algorithm}[t]
\caption{TDMA-CBAA frame execution (one tick, $\Delta t$)}
\label{alg:tdmacbaa}
\begin{algorithmic}[1]
...
\subsection{Phase I: Single-Task Consensus Auction}\label{sec:method-auction}
Each drone executes the CBAA phases locally, restricted to its perceived swarm $\mathcal{P}_i$:
...
This mechanism provides fault tolerance against departed agents, but as Section~\ref{sec:theory-noiseless}(ii) shows, it is also the mechanism through which belief expiry can destabilize consensus.
...
The released implementation is the single-task, bundle-size-one
member of the consensus-auction family, i.e., the Consensus-Based
Auction Algorithm (CBAA)~\cite{choi2009}. Each agent maintains one
active task and exchanges
winner--bid pairs through the communication layer. The implementation
therefore captures the consensus and communication dynamics relevant
to the present study, but does not claim to implement the full
multi-task bundle construction of canonical CBBA.
\subsection{Phase II: Motion Control}
Between auctions, drones execute a potential-field controller toward assigned objectives, away from SAM threats (Eq.~\eqref{eq:threat-potential}), and with Reynolds-style separation from neighbors within $4$\,m drawn from the perceived swarm $\mathcal{P}_i$ under TDMA.
An allocation-only ablation (Sec.~\ref{sec:setup-ablation}) with motion disabled confirms that MTC ordering across baselines is driven by the communication layer.
